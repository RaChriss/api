...
CREATE TABLE Signalement (
    ...
    location GEOMETRY (Point, 4326), -- Point géographique PostGIS (longitude, latitude)
...
-- Statuts
INSERT INTO
    Status (libelle, couleur)
VALUES ('Nouveau', '#FF0000'), -- Rouge
...
VALUES (
        'Entreprise Municipal',
        '+261 20 22 123 45',
        'municipal@antananarivo.mg',
...
SELECT
    ...
    COALESCE(
        ROUND(
            COUNT(
                CASE
                    WHEN r.Id_Status = 3 THEN 1
                END
            ) * 100.0 / NULLIF(COUNT(r.Id_Reparation), 0),
            2
        ),
        0
    ) as avancement_pct
...
SELECT
    ...
    s.firebase_id,
    s.est_synchronise,
    u.display_name as utilisateur_nom,
    u.email as utilisateur_email,
    u.firebase_uid as utilisateur_firebase_uid,
